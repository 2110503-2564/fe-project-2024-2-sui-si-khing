...
interface Booking {
  ...
  pickupDate: string;
  returnDate: string;
  provider: Provider; // Provider is now an object embedded in the booking
  status: string;
  paymentStatus: string; // Added paymentStatus field
}


export default function MyBookings({ token }: { token: string }) {
  ...
  const handleEditSubmit = async () => {
    if (!carModel || !pickupDate || !returnDate) {
    ...
    try {
      const response = await fetch(`http://localhost:5003/api/bookings/${editingBooking?._id}`, {
        ...
        body: JSON.stringify({ carModel, pickupDate, returnDate }),
      });
        ...
        setBookings((prevBookings) =>
          prevBookings.map((booking) =>
            booking._id === editingBooking?._id
              ? { ...booking, carModel, pickupDate, returnDate }
              : booking
          )
        );
  ...
  const handleDelete = (bookingId: string) => {
    setDeletingBookingId(bookingId); // Set the booking ID to be deleted
  ...
  const deleteBooking = async (bookingId: string) => {
    try {
      ...
      setDeletingBookingId(null); // Reset the deleting state
    }
  };
